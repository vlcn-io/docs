...
<div>
window.dbPromise = window.sqlite3InitModule().then((sqlite3) => {
  const db = new sqlite3.oo1.DB();
  db.exec([
    `CREATE TABLE node (id, parent_id, content, primary key (id));`,
    `CREATE INDEX node_parent_id ON node (parent_id);`
  ]);
...
dbPromise.then((db) => db.transaction(() => fill(db)));
</div>
</div>
...
return dbPromise.then((db) => {
  ...
  return js_beautify(JSON.stringify(cases));
});
</div>
</div>

On my machine, recursive sort was about 2x slower than regular sort in all cases and between 8-12x faster than the control.

The "2x slower **in all cases**" is also a good sign -- it means that the perf of a recursive sort tracks that of a run of the mill sort on an indexed column.
...
- While slower than a fractional index, it is still very fast
- Does not lose precision as a fractional index does. See [[blog/fractional-indexing]]
- Does now blow up in storage space as a fractional index can. See [[blog/fractional-indexing]]
- Inserting or moving a row only requires updating at most 3 rows. See [[blog/recursive-ordering]]

# Drawbacks

- The implementation shown here cannot be used in a distributed setting where nodes write without coordination. To solve this, see [[blog/distributed-recursive-ordering]]. You can of course use it in a traditional client-server setting. 
