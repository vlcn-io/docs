...
title: "CRDT Substrate: Creating Convergence Without Specialized Knowledge"
...
1. Your mutations must be deterministic. No generating random data inside of them although you can pass random data as an input.
...
Some example mutations have been provided for you below.
...
Below are three independent UIs that are driven by three indepent copies of the initial state. On each one you can run your mutations. Whenever you desire, have the UIs merge their state together by pressing the "sync nodes" button.

<br/>
<Nodes />

Pretty cool, right? All of the nodes converge to the same state on merge and you didn't have to write any special code to make this happen. You just wrote some mutations and the substrate took care of the rest.
...
How do we preseve the user's intent and how does the mathematical definition of convergence deviate from pratical expectations?
...
At some point we might ship the "crdt substrate" idea to lower the barrier to entry to, and provide an escape hatch to, author custom convergent data types.
...
In this setup, each process can locally write to it's log without coordinating with others. All it needs to do when writing is create a new event whose parents are the leaves of it's current copy of the log. In our interactive example, each mutation you defined is decorated such that it calls `addEvent` when being invoked.
...
Since all processes share a common ancestor for their DAG (the sentinel "ROOT" element), their DAGs will be [connected](https://mathworld.wolfram.com/ConnectedGraph.html) after a merge. This ensures no events are ever lost by being orphaned.
...
Which brings us to the last trick -- updating the current application state based on the merged DAG.
...
But how do you re-play a log that is not linear? You make it linear. Each process does a deterministic [breadth first traversal](https://en.wikipedia.org/wiki/Breadth-first_search) of the DAG to create a single sequence of events. This interpretation of events is what is applied to the state snapshot.
...
**Given this model is driven by a bunch of operations, is it really a CRDT?**

I think so. All CRDTs have a merge operation. In our case, that merge operation is the merging of the DAG + replaying of the transaction log. Running the merge operation generates the application state.

One could argue that the CRDT is only the DAG + Defined Mutations and that application state is merely a side-effect of the merge operation of that CRDT. In either case, it [fulfills the properties of a CRDT](https://en.wikipedia.org/wiki/Conflict-free_replicated_data_type#State-based_CRDTs):

- The merge function is:
  - Commutative
  - Associative
  - Idempotent
- Mutations form a [semilattice](https://en.wikipedia.org/wiki/Semilattice) by writing events to the DAG.
