...
[cr-sqlite](/cr-sqlite/intro) is network agnostic. You can use [crsql_changes](/cr-sqlite/api-methods/crsql_changes) to pull changes from one database, ship them over any protocol you like, then use [crsql_changes](/cr-sqlite/api-methods/crsql_changes) on the other end to apply the changes. In any case, a WebSocket implementation is provided for convenience [here](/js/networking) and an overview of writing your own network code [here](./whole-crr-sync).
