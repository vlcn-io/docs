...
Now that we have a data model sketched out we can apply it to a database. Let's import the `crsqlite-wasm` module and open a database in memory. Too learn how to persist data to disk, see [/docs/js/persistence](/docs/js/persistence).

<RunnableCode
  code={`const initWasm = await import('https://esm.sh/@vlcn.io/crsqlite-wasm@0.14.0-next.0');
const sqlite = await initWasm.default(() => "https://esm.sh/@vlcn.io/crsqlite-wasm@0.14.0-next.0/dist/crsqlite.wasm");
...
- [crsql_siteid](/docs/cr-sqlite/api-methods/crsql_siteid)
- [crsql_changes](/docs/cr-sqlite/api-methods/crsql_changes)
...
- [A Websocket sync server & client](/docs/js/networking)
