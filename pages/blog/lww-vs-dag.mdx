...
We know that a grow only set of LWW registers is a CRDT from the [last article](/blog/gentle-intro-to-crdts) but how do we turn an event log into a CRDT? And how do we process it consistently so every node arrives at the same state after processing the log?
