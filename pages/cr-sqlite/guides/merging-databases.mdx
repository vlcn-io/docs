...
The base primitives around which everything is built are tables and `crrs` ([conflict free replicated relations](/concepts/crr)).
...
Now that we have a data model sketched out we can apply it to a database. Let's import the `crsqlite-wasm` module and open a database in memory. Too learn how to persist data to disk, see [guides/persistence](./persistence).
...
As you can see, all the tables were created as expected. There are a few internal tables as well which are used by crsqlite to track merge state. You can read more about those at [bits/internal-tables](/cr-sqlite/bits/internal-tables).
...
The main primitive for this is [crsql_changes](../api-methods/crsql_changes) which allows you to:
...
`crsqlite` maintains a [crsql_tracked_peers](../api-methods/crsql_tracked_peers) table that tracks the `db_version` of each peer. Let's see what that looks like for each database.
...
Updating `tracked_peers` on read is a responsibility of network layers that need to stream changes. See [crsql_tracked_peers](../api-methods/crsql_tracked_peers).
...
- [crsql_siteid](../api-methods/crsql_siteid)
- [crsql_changes](../api-methods/crsql_changes)
- [crsql_tracked_peers](../api-methods/crsql_tracked_peers)
