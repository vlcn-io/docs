...
A function to upgrade existing tables to "conflict free replicated relations", or [crrs](../concepts/crr), for short.
...
```sql
SELECT * FROM crsql_changes WHERE db_version > :v AND site_id IS NULL
```

2. Get all changes excluding those synced from `some_site` after version `v`

```sql
SELECT * FROM crsql_changes WHERE db_version > :v AND site_id != :some_site
```
...
That was a whirlwind. You can see a [line-by-line breakdown of using cr-sqlite](https://observablehq.com/@tantaman/cr-sqlite-basic-setup) as well as continue [to the tutorials](./guides/merging-databases) & [api documentation](./api-methods/crsql_as_crr).
