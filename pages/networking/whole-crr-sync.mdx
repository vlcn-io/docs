...
`crsqlite` provides a [crsql_tracked_peers](/cr-sqlite/api-methods/crsql_tracked_peers) table that networking layers can use to track the `db_version` of each peer. Networking layers are of course allowed to create their own tables to track this information as well.
...
<RunnableCode
  code={`use deckid;
return await db1.execA(\`SELECT * FROM crsql_changes WHERE db_version > ? AND site_id IS NULL\`, [4]);`}
/>
...
- A REST sync server
- P2P sync over a lossy pub-sub channel

are in progress.

Reference implementations of:

- [A Websocket sync server & client](/js/networking)
- [Simple P2P sync over WebRTC](https://github.com/vlcn-io/cr-sqlite/tree/main/js/packages/p2p)
